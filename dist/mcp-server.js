...
const TOOLS = [
    {
        name: "scrape_url",
        description: "Scrape any public URL and return structured data: text, links, images, and metadata. " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                javascript: {
                    type: "boolean",
                    default: false,
                    description: "Force JavaScript rendering via headless browser. Use for SPAs or pages that require JS to load content.",
                },
                proxy_country: {
                    type: "string",
                    description: "ISO 3166-1 alpha-2 country code to geo-target the scrape (e.g. US, GB, KE, DE)",
                },
                wait_for: {
                    type: "string",
                    description: "CSS selector to wait for before extracting content. Useful for lazy-loaded content.",
                },
                timeout: {
                    type: "number",
                    default: 30000,
    ...
    },
    {
        ...
        description: "Scrape a URL and extract specific fields using CSS selectors. " +
                ...
                fields: {
                    ...
                    items: {
                        ...
                        properties: {
                            ...
                            attribute: {
                                type: "string",
                                description: "HTML attribute to extract (e.g. href, src, content). Omit to get text content.",
                            },
                        },
                    },
                },
    ...
    },
    {
        ...
        description: "Take a full-page screenshot of a URL and return it as a base64-encoded PNG. " +
    ...
    {
        name: "check_credits",
        description: "Check the remaining scrape credits on the current API key. " +
    ...
    {
        name: "list_datasets",
        description: "List all available pre-built datasets. " +
        ...
        inputSchema: {
            ...
            properties: {
                category: {
                    type: "string",
                    description: "Filter by category: Jobs & Labor, E-Commerce, Real Estate, Finance & VC, Social & Media",
                },
            },
        },
    },
...
function formatScrapeResult(data, meta) {
    const lines = [];
    if (meta?.billing) {
    ...
    if (data.status)
        lines.push(`HTTP Status: ${data.status}`);
    lines.push("");
    if (data.text) {
    ...
    if (data.links?.length) {
        ...
        data.links.slice(0, 30).forEach((l) => lines.push(l));
        if (data.links.length > 30)
            lines.push(`...and ${data.links.length - 30} more`);
        lines.push("");
    }
    if (data.images?.length) {
        lines.push(`--- IMAGES (${data.images.length}) ---`);
        data.images.slice(0, 20).forEach((i) => lines.push(i));
        lines.push("");
    }
...
async function handleScrapeMultiple(args) {
    const urls = args.urls;
    const results = await Promise.allSettled(urls.map(url => apiRequest("/api/scrape", {
        url,
        method: args.javascript ? "headless" : "http",
        javascript: args.javascript ?? false,
        extract: {
...
async function handleScreenshot(args) {
    ...
    if (result.data.metadata?.title)
        lines.push(`Title: ${result.data.metadata.title}`);
    if (result.meta?.billing)
        lines.push(`Credits used: ${result.meta.billing.creditsUsed}`);
...
async function handleListDatasets(args) {
    const datasets = [
...
// ── Server setup ─────────────────────────────────────────────────────────────
const server = new index_js_1.Server({ name: "scrapeapi", version: "1.0.0" }, { capabilities: { tools: {} } });
    ...
    try {
        ...
        switch (name) {
            ...
            case "scrape_multiple":
                result = await handleScrapeMultiple(args);
                break;
            case "extract_structured":
                result = await handleExtractStructured(args);
                break;
            case "take_screenshot":
                result = await handleScreenshot(args);
                break;
            case "check_credits":
                result = await handleCheckCredits();
                break;
            case "list_datasets":
                result = await handleListDatasets(args);
                break;
            default:
